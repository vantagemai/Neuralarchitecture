      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-2 gap-4">
        ...
        <div className="bg-surface border border-b1 rounded-lg p-4">
          ...
          <div className="space-y-4">
            <div>
              <label className="text-xs text-t3 uppercase tracking-wider font-semibold">Nome completo</label>
            ...
            <div>
              <label className="text-xs text-t3 uppercase tracking-wider font-semibold">Email</label>
              <input type="email" value={email} onChange={e => setEmail(e.target.value)} placeholder="joao@email.com"
                className="mt-1 w-full bg-elevated border border-b1 rounded-lg px-4 py-2.5 text-sm outline-none focus:border-vred/40" />
            </div>
            <div>
              <label className="text-xs text-t3 uppercase tracking-wider font-semibold">Funcao</label>
            ...
            <div>
              <label className="text-xs text-t3 uppercase tracking-wider font-semibold">Senha inicial</label>
                ...
                <div key={u.id} className="bg-surface border border-b1 rounded-lg px-3 py-1.5 hover:border-b3 transition-colors">
                  {editId === u.id ? (
                    ...
                    <div className="flex items-center gap-3">
                      <Avatar userId={u.id} name={u.name} size="w-9 h-9" />
                      <div className="flex-1 min-w-0">
                        <Link to={`/perfil/${u.id}`} className="text-sm font-semibold truncate hover:text-vred transition-colors block">{u.name}</Link>
                        <div className="text-xs text-t4 font-mono">{u.email}</div>
                      </div>
                      <Badge variant={roleVariant(u.role)}>{u.role}</Badge>
                      <span className="text-2xs text-t4 font-mono">🪙 {getTotalXp(u.id)}</span>
                      {getStreak(u.id).current > 0 && <span className="text-2xs text-orange-400">🔥{getStreak(u.id).current}</span>}
